...
model = joblib.load('svm_classifier_linear_ver4.dat')
array_names = ["Kit", "Bong", "Boyet", "Guard", "Henry", "Janet","Jay","Jerlyne","Jessy","Joven","Mohaimen","Randolf","Sherry"]
...
text1 = tk.Message(sidebar, textvariable=var, bg='white', width=300, font=('', 7))
...
def create_window():
    ...
    otext1 = tk.Message(osidebar, textvariable=ovar, bg='white', width=300, font=('', 7))
...
def run_recognition():
    ...
    print(indx)
    out = prediction[0][indx]
    if out < THRESHOLD:
        prediction_text = "UNKNOWN"
    else:
        prediction_text = "Prediction: " + str(array_names[np.argmax(prediction)])
    var1.set(prediction_text)
    ovar1.set(prediction_text)
    threading.Thread(target=create_window).run()
